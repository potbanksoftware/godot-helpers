...
## Adds an item, with text label and (optionally) id.
## If no id is passed, the item index will be used as the item's ID. New items are appended at the end.
##
...
## Adds a separator to the list of items.
## Separators help to group items, and can optionally be given a text header.
##
## A separator also gets an index assigned, and is appended at the end of the item list.
